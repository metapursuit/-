...
库用法：
  ...
  h = Hippocampus()                       # 默认 hermes-companion-configs/reflex_events.json（事实源真身）
...
# 事实源唯一性：reflex_events.json 真身在 hermes-companion-configs（git 管理）。
# 2026-08-12 统一：AppData 默认路径从不放文件，所有调用曾显式传 --file；
# 现改为默认直指仓库真身，杜绝"默认路径读空"隐患。迁移事件/路径改动须同步 MEMORY 与 brain-organ-dev skill。
def _default_events_path():
    """默认事件路径：环境变量 companion_EVENTS 优先；否则从本文件向上找含
    reflex_events.json 的目录（真身=仓库根）；找不到则退回当前目录。
    分发/副本场景请用 --file 或设置 companion_EVENTS，避免静默读空或指向打包者机器。"""
    env = os.environ.get("companion_EVENTS")
    if env:
        return env
    ...
    for _ in range(8):
        ...
        if up == d:
            break
        d = up
    return os.path.join(os.getcwd(), "reflex_events.json")


DEFAULT_PATH = _default_events_path()
...
class Hippocampus:
    ...
    def set_status(self, eid, status):
        ...
        恢复(reactivate)时若该事件已被提炼游标越过(id <= last_refined)，
        回退游标使其重新进入提炼池——否则被抑制过的事件会永久丢失。"""
        for e in self.data["events"]:
            if e["id"] == eid:
                e["status"] = status
                if status == "active":
                    cursor = self.data["meta"].get("last_refined") or 0
                    if eid <= cursor:
                        self.data["meta"]["last_refined"] = eid - 1
                self.save()
                return e
        raise KeyError(f"event {eid} not found")

    def update(self, eid, **fields):
        """任意字段更新——杏仁核写回情绪标注用。改 score 会刷新正负计数。"""
        for e in self.data["events"]:
            if e["id"] == eid:
                e.update(fields)
                self._refresh_meta()
    ...
    def set_cursor(self, value):
        """设置提炼游标——前额叶专用，取代直接操作内部 data。"""
        self.data["meta"]["last_refined"] = value
        self.save()
    ...
    def query(self, tag=None, category=None, scene_type=None,
        ...
        # 注：tag 为子串匹配（搜"气"会命中"语气"）——有意为之，支持未来多标签
        out = []
        for e in self.data["events"]:
    ...
    def stats(self):
        ...
        return {
            ...
            "neutral": len(events) - m["total_positive"] - m["total_negative"],
            "by_category": by_cat,
...
def _coerce(v):
    """字符串 → int/float/bool/None/str 的类型推断。"""
    v = v.strip()
    if v in ("true", "True"):
        return True
    if v in ("false", "False"):
    ...
    try:
        ...
        pass
    # JSON 字面量（如 confidence_interval=[-8,-6]）——修复 update --set 存字符串 bug
...
def _parse_ci(s):
    """解析 --ci "lo,hi"。非法输入返回 None 并提示，不裸崩。"""
    if not s:
        return None
    try:
        parts = [int(x.strip()) for x in s.split(",")]
        if len(parts) != 2:
            print(f"warning: --ci 需要两个数字 lo,hi，收到 {len(parts)} 个，忽略", file=sys.stderr)
            return None
        return parts
    except ValueError:
        print(f"warning: --ci '{s}' 含非数字，忽略", file=sys.stderr)
        return None
...
def main():
    ...
    a.add_argument("--ci", help="置信区间 lo,hi")
    ...
    u = sub.add_parser("update", help="更新字段（杏仁核标注用）")
    u.add_argument("--id", type=int, required=True)
    u.add_argument("--set", action="append", required=True, help="key=value，可重复")

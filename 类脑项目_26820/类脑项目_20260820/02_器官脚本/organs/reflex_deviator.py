...
# 2026-08-20 分发适配：环境变量 companion_EVENTS 优先，否则向上探测，避免指向打包者机器。
...
def main():
    ...
    p.add_argument("--file", default=EVENTS_PATH,
                   help="reflex_events.json 路径（默认自动探测，可设环境变量 companion_EVENTS）")
    args = p.parse_args()

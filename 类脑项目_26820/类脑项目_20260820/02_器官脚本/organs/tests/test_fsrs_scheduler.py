...
def main():
    ...
    # next_audit 必须是真正的未来时刻（修复 2026-08-20：此前为 now + 字符串后缀）
    ok = len(st["rules"]) == 4 and all(r2["S"] == 7.0 for r2 in st["rules"].values()) \
         ...
         and all(datetime.fromisoformat(r2["next_audit"]) > now for r2 in st["rules"].values())
    results.append((f"schedule: 4规则 S=7 next={exp_next}d 且 next_audit>now", ok))

...
def mk(eid, score, cat, tag, status="active", ts=None):
    if ts is None:
        # 默认"昨天"：保证在 CLUSTER_WINDOW_DAYS=7 窗口内（2026-08-20 修复：
        # 原写死 2026-08-11，8/20 后跑全部超窗被滤成空列表）
        from datetime import datetime, timedelta, timezone
        ts = (datetime.now(timezone(timedelta(hours=8))) - timedelta(days=1)).isoformat()

...
- 2026-08-20T20:5x | @session:default/本会话 | 类脑项目全量打包交付（桌面 zip）
  - 做了：按"脚本器官+说明+日志+修改过程+目的+将来期望"六类打包——organs/ 七器官+词库+状态+tests（57 文件 392KB）→ Desktop\【AI伙伴】类脑项目_20260820.zip（162KB，UTF-8 名兼容解压）；手写 README.md（架构/状态/过程/期望）+ 06_将来期望\未来规划.md（CoBRA 深化/词库迭代/统计升级/九学三层）；git log 57 条导出入包；staging 已清理
  - 关键决策：打包源=hermes-companion-configs 真身+桌面既有审查文档+三 skill 完整目录；知识/数据不混淆（管线纯化原则不变）；zip 内根目录用中文名，UTF-8 flag 已验证
  - 遗留：无（快照性质，不取代 git 真身）

- 2026-08-20T21:0x | @session:default/本会话 | 打包脱敏：名字占位符化
  - 做了：Desktop\类脑项目_20260820.zip（脱敏版）——【用户】→【用户】、【AI伙伴】→【AI伙伴】（含"【AI伙伴】注"等组合）、companion→companion（文件名/skill 名/commit 消息）、user→user（路径）；41 文件内容替换 + 4 处重命名（companion-memory-architecture.html / companion-contract-v2.md / companion-agent-architecture / 根目录去"【AI伙伴】"前缀）；包内残留扫描干净；API key/内部服务地址/账号密码扫描干净；旧版 zip 已删
  - 关键决策：换名字必须连带拼音/路径变体（companion/user），否则等于没换；脱敏只作用于分发快照，git 真身原名不动；LOG 会话 ID（13 处）保留未模糊，如需外分享再处理
  - 遗留：会话 ID 是否模糊待【用户】定；zip 是真身快照，后续迭代需重打

- 2026-08-20T21:2x | @session:default/本会话 | 外部 agent 审计（子代理）→ 真身修复 + 副本 v2 重打包
  - 做了：Claude Code 调度失败（alwaysThinkingEnabled+effortLevel xhigh → thinking 参数与 DeepSeek reasoning_text 冲突，400；modelOverrides 可关 thinking 但触发 Not logged in；settings 已恢复原样——**code-reviews 审查工作流当前断链，待修**）；科大 API 也不可达（疑似校园网限制）。改用 delegate_task 子代理冷启动审计（deleg_67b7f574，28 API calls），报告 Desktop\code-reviews\20260820_类脑项目_外部agent分析.md
  - 审计发现（人工核验后）：4 个真 bug 实锤——FSRS next_audit 死时间戳（fsrs:132 写 now+字符串后缀）、confidence_interval 死字段（7 器官零消费）、prefrontal P_i 权重和 0.7、median 非标准定义 + FSRS D 死值；测试自包含声明不实（REAL 依赖真身路径）；self_check 布局假设不兼容副本；杏仁核 3/6 盲区（已知）；"69 项"构成=11+10+10+12+5+18+3 ✓（子代理漏数 run_amygdala 3 项）
  - 修复（git 876082d，12 文件）：next_audit→真正未来 ISO 时间戳+补断言；P_i 归一化(/0.7)；median 标准定义；测试 fixture 化（tests/data/reflex_events.json 脱敏版，4 文件 REAL 改指向 fixture）；reflex_deviator 加 --file；hippocampus/reflex_deviator 默认路径改 companion_EVENTS+自动探测；self_check 参数化(--events/--organs/--skill+探测+非 git 仓库跳过)；test_reflex_deviator 时间戳改动态"昨天"（8/11 写死时间戳 8/20 超窗挂）；README 已知限制诚实声明（CI 预留字段/D 待标定/n=8 insufficient/7 天窗口）
  - 副本 v2 重打包：Desktop\类脑项目_20260820.zip（58 条目 170KB，修正 v1 双层嵌套根目录）；解压副本验证 69/69 测试全绿 + self_check 116/116 全绿 + 器官 CLI 正常 + 脱敏扫描干净
  - 关键决策：发 GitHub 的副本必须自包含可运行（fixture 化+路径参数化），脱敏后自检通过才算交付；审计发现按"真 bug 修/已知项标注/脱敏副作用澄清"三分处理
  - 遗留：Claude Code thinking 配置问题（审查工作流断链，需 Cherry Studio 侧或 Claude Code 升级解决）；科大 API 不可达待确认；brain-organ-development skill SKILL.md 待同步（self_check 参数化说明）
